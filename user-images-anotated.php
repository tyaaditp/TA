...
$query = "SELECT image_original.id as 'original_id', image_original.user_id as 'original_user_id', image_original.image as 'original_image', image_anotated.id as 'anotated_id', image_anotated.user_id as 'anotated_user_id', image_anotated.image_id as 'anotated_reference', image_anotated.image as 'anotated_image', image_anotated.analisis as 'anotated_analisis'  from image_original INNER JOIN image_anotated ON image_original.id=image_anotated.image_id AND image_anotated.user_id = " . $user_id . " WHERE image_original.image = '/uploads/".$image_ori."'" ;

$sql2 = mysqli_query($link, $query);
if(mysqli_num_rows($sql2)>0) {
    while ($row2 = mysqli_fetch_assoc($sql2)) {
        $querySelectFirstAnotated = "select * from image_anotated where image='".$image_anotated."' and user_id=".$row2['original_user_id']." limit 1";
